...
test("curated upload → analyze → fold → inspect → export → bundle", async ({ page }, testInfo) => {
  ...
  await expect(analysisRegion.getByText("safe to fold", { exact: true })).toBeVisible();
  ...
  await page.getByRole("link", { name: "Download .photofold" }).click();
